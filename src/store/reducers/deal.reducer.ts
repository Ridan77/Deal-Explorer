...
export type DealAction =
  | { type: typeof SET_FILTER; filterBy: FilterBy }
  | { type: typeof SAVE_DEAL; dealId: string }
  | { type: typeof UNSAVE_DEAL; dealId: string }
...
export function dealReducer(
  ...
  switch (action.type) {
    ...
    case UNSAVE_DEAL: {
      const savedDeals = state.savedDeals.filter((id) => id !== action.dealId)
      newState = { ...state, savedDeals }
      break
    }
    default:
